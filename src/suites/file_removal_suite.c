...
#define FILENAME_TEMPLATE "file_removal_suite_XXXXXX"

typedef struct {
    char * filename;
    int * results;
    size_t nb_threads;
} file_removal_mt_data;

static int file_removal_mt_init(void ** test_suite_data,
                                size_t const nb_threads)
{
    int result = 0;
    assert(test_suite_data != NULL);
    file_removal_mt_data * data = malloc(sizeof *data);
...
static int file_removal_mt_run(void * test_suite_data,
                               size_t id)
{
    int result = 0;
    file_removal_mt_data * data = test_suite_data;
...
static int file_removal_mt_post_run(void * test_suite_data)
{
    int result = 0;
    file_removal_mt_data * data = test_suite_data;
...
static int file_removal_mt_deinit(void * test_suite_data)
{
    int result = 0;
    file_removal_mt_data * data = test_suite_data;
...
test_suite const test_suite_file_removal_mt =
{
    "file_removal_mt",
    &file_removal_mt_init,
    &file_removal_mt_run,
    &file_removal_mt_post_run,
    &file_removal_mt_deinit,
    test_suite_type_mt
};
